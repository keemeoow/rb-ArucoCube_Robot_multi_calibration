...
def auto_detect_dynamic_cams(intrinsics_dir: Path) -> Tuple[int, ...]:
    """`intrinsics_dir/device_map.json` 의 `gripper_cam_idx` 를 dynamic cam id 로
    추출. 파일이 없거나 키가 없으면 빈 tuple 반환."""
    p = Path(intrinsics_dir) / "device_map.json"
    if not p.exists():
        return ()
    try:
        with open(p, "r") as f:
        ...
        return ()
    idx = m.get("gripper_cam_idx", None)
    if idx is None:
        return ()
    return (int(idx),)
...
def load_calibration(data_dir: Path, intrinsics_dir: Path,
    ...
    """카메라 intrinsic + extrinsic 로드 (정적 + 동적 그리퍼 캠 지원).

    Parameters
    ----------
    num_cams       : None 이면 `intrinsics_dir/cam*.npz` 개수로 자동 감지.
    dynamic_cam_ids: 그리퍼처럼 매 프레임 외부 파라미터가 바뀌는 캠 id 들.
                     해당 캠은 `T_C0_C{ci}.npy` 대신 hand-eye + 매 프레임
                     `T_base_ee_<frame>.npy` 를 곱해 cam0 기준 extrinsic 을 만든다.
    calib_subdir   : `data_dir` 안에서 캘리브레이션 산출물(npy)이 저장된 폴더.

    Returns
    -------
    intrinsics     : List[CameraIntrinsics]
    extrinsics     : Dict[int, np.ndarray]    # 정적 캠만 채워짐 (cam0 = I)
    dynamic_calib  : Dict[int, Dict[str, np.ndarray]]
        각 dynamic cam_id 마다 {"T_C0_base": (4,4), "T_ee_cam": (4,4)}.
        load_frame 이 매 프레임 T_base_ee 와 곱해 cam0 기준 extrinsic 을 계산.
    """
    ...
    dynamic_set = {int(c) for c in dynamic_cam_ids}
    ext_dir = Path(data_dir) / calib_subdir
    extrinsics = {0: np.eye(4)}
    for ci in range(1, num_cams):
        if ci in dynamic_set:
            continue  # 매 프레임 계산
    ...
    if dynamic_set:
        if 0 in dynamic_set:
            raise ValueError("cam0 cannot be a dynamic gripper cam (used as world).")
        T_base_C0_path = ext_dir / "T_base_C0.npy"
        T_gripper_cam_path = ext_dir / "T_gripper_cam.npy"
        if not T_base_C0_path.exists():
            raise FileNotFoundError(
                f"missing robot-base→cam0 extrinsic for dynamic cam: {T_base_C0_path}")
        if not T_gripper_cam_path.exists():
            raise FileNotFoundError(
                f"missing hand-eye T_gripper_cam: {T_gripper_cam_path}")
        T_base_C0 = np.load(str(T_base_C0_path)).astype(np.float64)
        T_C0_base = np.linalg.inv(T_base_C0)
        T_ee_cam = np.load(str(T_gripper_cam_path)).astype(np.float64)
        for ci in sorted(dynamic_set):
            if ci >= num_cams:
                raise ValueError(f"dynamic cam id {ci} >= num_cams ({num_cams})")
            dynamic_calib[ci] = {"T_C0_base": T_C0_base, "T_ee_cam": T_ee_cam}
    return intrinsics, extrinsics, dynamic_calib
...
def load_frame(data_dir: Path, frame_id: str, intrinsics, extrinsics,
               ...
               num_cams: Optional[int] = None,
               dynamic_cam_ids=(),
               dynamic_calib: Optional[Dict[int, Dict[str, np.ndarray]]] = None):
    """frame_id 의 RGB-D 를 모든 카메라에서 로드.

    dynamic_cam_ids 에 포함된 캠은 `cam{ci}/T_base_ee_<frame>.npy` 를 읽어
    `T_C0_cam = T_C0_base @ T_base_ee @ T_ee_cam` 로 매 프레임 extrinsic 을 계산.
    """
    if num_cams is None:
        num_cams = len(intrinsics)
    dynamic_set = {int(c) for c in dynamic_cam_ids}
    if dynamic_set and dynamic_calib is None:
        raise ValueError("dynamic_calib must be provided when dynamic_cam_ids is non-empty")
    ...
    for ci in range(num_cams):
        ...
        if ci in dynamic_set:
            ee_path = img_dir / f"cam{ci}" / f"T_base_ee_{frame_id}.npy"
            if not ee_path.exists():
                raise FileNotFoundError(
                    f"missing per-frame EE pose for dynamic cam{ci}: {ee_path}")
            T_base_ee = np.load(str(ee_path)).astype(np.float64)
            calib = dynamic_calib[ci]
            T_C0_cam = calib["T_C0_base"] @ T_base_ee @ calib["T_ee_cam"]
            frames.append(CameraFrame(ci, intrinsics[ci], T_C0_cam, c, d))
        else:
...
def run_pipeline(data_dir, intrinsics_dir, frame_id,
    ...
    # 1. 캘리브레이션 + GLB (자동 탐색)
    intrinsics, extrinsics, _ = load_calibration(data_dir, intrinsics_dir)
