...
from Obj_pose_pipeline import (
    CameraIntrinsics, CameraFrame,
    load_calibration, load_frame, auto_detect_dynamic_cams,
...
# ═══════════════════════════════════════════════════════════
# 8.5  Tier-3 pose quality gate (offline, post-pose)
# ═══════════════════════════════════════════════════════════

DEFAULT_POSE_GATE_CFG = {
    "fitness_min":         0.70,    # ICP fitness (높을수록 좋음)
    "rmse_max_m":          0.005,   # ICP inlier RMSE (낮을수록 좋음, m)
    "silhouette_iou_min":  0.50,    # 렌더 silhouette ∩ SAM mask IoU
    "scale_lo":            0.70,    # 추정 scale 하한 (canonical 기준)
    "scale_hi":            1.50,    # 추정 scale 상한
    "min_pair_angular_spread_deg": 30.0,  # 카메라 광축 쌍 사이 최대 각도
}


def _camera_optical_axis(cam) -> np.ndarray:
    """카메라 광축 방향 (cam0 frame). T_base_cam[:3,:3] @ +Z."""
    R = cam.T_base_cam[:3, :3]
    z = R @ np.array([0.0, 0.0, 1.0])
    n = np.linalg.norm(z)
    return z / n if n > 1e-9 else z


def evaluate_viewpoint_diversity(frames) -> Dict[str, Any]:
    """카메라 광축 쌍 사이의 각도 (deg). max 가 클수록 triangulation 유리."""
    if len(frames) < 2:
        return {"max_pair_deg": 0.0, "min_pair_deg": 0.0, "n_pairs": 0,
                "pairs": []}
    axes = [(cam.cam_id, _camera_optical_axis(cam)) for cam in frames]
    pairs = []
    for i in range(len(axes)):
        for j in range(i + 1, len(axes)):
            ci, ai = axes[i]; cj, aj = axes[j]
            dot = float(np.clip(np.dot(ai, aj), -1.0, 1.0))
            ang = float(np.degrees(np.arccos(dot)))
            pairs.append({"cams": [int(ci), int(cj)], "angle_deg": ang})
    angs = [p["angle_deg"] for p in pairs]
    return {"max_pair_deg": max(angs), "min_pair_deg": min(angs),
            "n_pairs": len(angs), "pairs": pairs}


def _scene_center(frames) -> np.ndarray:
    """카메라 위치 평균을 'object 가 있을만한 곳'의 추정으로 사용. base frame."""
    pts = np.stack([cam.T_base_cam[:3, 3] for cam in frames])
    return pts.mean(axis=0)


def recommend_viewpoint_for_diversity(frames,
                                       target_spread_deg: float = 60.0,
                                       distance_m: float = 0.30,
                                       n_grid: int = 24) -> Dict[str, Any]:
    """현재 카메라 광축들로부터 가장 'novel' 한 시선 방향을 단위구면에서 탐색.

    그리퍼 캠을 그 방향으로 옮기면 max_pair_deg 가 가장 크게 증가. 결과는:
      - axis_in_base    : 추천 시선 방향 (cam → object 방향, base frame)
      - cam_position_m  : object 중심에서 -axis * distance 만큼 떨어진 점
      - look_at_m       : object 중심 (=scene_center)
      - azimuth_deg     : XY 평면에서의 방위각 (Z up 기준)
      - elevation_deg   : 평면 위로 들어올린 각도
      - expected_max_pair_deg_after : 이 시점 추가 시 예상 max pair angle
    """
    if len(frames) == 0:
        return {"available": False}
    existing = np.stack([_camera_optical_axis(cam) for cam in frames])  # (N,3)
    # 단위구면에 azimuth/elevation 그리드 샘플
    az = np.deg2rad(np.linspace(-180, 180, n_grid, endpoint=False))
    el = np.deg2rad(np.linspace(-60, 80, max(2, n_grid // 2)))
    A, E = np.meshgrid(az, el)
    cx = np.cos(E) * np.cos(A)
    cy = np.cos(E) * np.sin(A)
    cz = np.sin(E)
    candidates = np.stack([cx.ravel(), cy.ravel(), cz.ravel()], axis=1)  # (M,3)

    # 각 후보가 기존 광축들과 이루는 최대각 — 클수록 novel 함
    # candidate 가 시선 방향(카메라→물체) 일 때 cam_axis = candidate 로 간주
    # (positive Z 방향이 카메라 시선)
    # angle(c, e_i) = arccos(c · e_i)
    dots = np.clip(candidates @ existing.T, -1.0, 1.0)  # (M,N)
    angs = np.degrees(np.arccos(dots))                    # (M,N)
    min_to_existing = angs.min(axis=1)                    # (M,)
    best_idx = int(np.argmax(min_to_existing))
    best_axis = candidates[best_idx]                      # (3,)
    best_min = float(min_to_existing[best_idx])

    # expected max_pair_deg 시뮬: 이 시점 추가했을 때 모든 쌍 중 max
    new_axes = np.vstack([existing, best_axis[None, :]])
    pair_dots = np.clip(new_axes @ new_axes.T, -1.0, 1.0)
    pair_angs = np.degrees(np.arccos(pair_dots))
    np.fill_diagonal(pair_angs, 0.0)
    expected_max = float(pair_angs.max())

    # azimuth/elevation 표시값
    az_deg = float(np.degrees(np.arctan2(best_axis[1], best_axis[0])))
    el_deg = float(np.degrees(np.arcsin(np.clip(best_axis[2], -1.0, 1.0))))

    # 카메라가 위치할 점 (object 중심 - axis * distance)
    obj_center = _scene_center(frames)
    cam_pos = obj_center - best_axis * float(distance_m)

    return {
        "available": True,
        "axis_in_base": [float(x) for x in best_axis],
        "cam_position_m": [float(x) for x in cam_pos],
        "look_at_m": [float(x) for x in obj_center],
        "azimuth_deg": az_deg,
        "elevation_deg": el_deg,
        "distance_m": float(distance_m),
        "min_angle_to_existing_deg": best_min,
        "expected_max_pair_deg_after": expected_max,
        "target_spread_deg": float(target_spread_deg),
        "meets_target": expected_max >= float(target_spread_deg),
        "hint": "그리퍼 캠을 cam_position_m 위치에서 look_at_m 을 향해 두면 "
                "viewpoint diversity 가 {:.1f}deg 까지 올라감".format(expected_max),
    }


def evaluate_pose_gate(pose: Optional[Dict[str, Any]],
                        cfg: Dict[str, float]) -> Dict[str, Any]:
    """단일 물체 pose 결과를 평가. pose=None 이면 자동 fail."""
    if pose is None:
        return {"pass": False, "reasons": ["pose_failed"], "metrics": {}}
    fitness = float(pose.get("fitness", 0.0))
    rmse = float(pose.get("rmse", 1.0))
    iou = float(pose.get("silhouette_iou", 0.0))
    scale = float(pose.get("scale", 0.0))
    reasons = []
    if fitness < cfg["fitness_min"]:
        reasons.append("fitness({:.3f}<{:.3f})".format(fitness, cfg["fitness_min"]))
    if rmse > cfg["rmse_max_m"]:
        reasons.append("rmse({:.4f}m>{:.4f}m)".format(rmse, cfg["rmse_max_m"]))
    if iou < cfg["silhouette_iou_min"]:
        reasons.append("silhouette_iou({:.3f}<{:.3f})".format(iou, cfg["silhouette_iou_min"]))
    if scale < cfg["scale_lo"] or scale > cfg["scale_hi"]:
        reasons.append("scale({:.2f}∉[{:.2f},{:.2f}])".format(
            scale, cfg["scale_lo"], cfg["scale_hi"]))
    return {
        "pass": len(reasons) == 0,
        "reasons": reasons,
        "metrics": {"fitness": fitness, "rmse_m": rmse,
                     "silhouette_iou": iou, "scale": scale},
    }


def build_quality_report(frame_id: str, profiles, poses: Dict[str, Any],
                          frames, cfg: Dict[str, float]) -> Dict[str, Any]:
    """frame 단위 종합 리포트.
    - per-object pose gate
    - viewpoint diversity
    - overall pass = (모든 object pose gate pass) AND (viewpoint diversity pass)
    """
    per_object = {}
    object_pass = True
    for prof in profiles:
        rep = evaluate_pose_gate(poses.get(prof.name), cfg)
        per_object[prof.name] = rep
        if not rep["pass"]:
            object_pass = False

    vp = evaluate_viewpoint_diversity(frames)
    vp_pass = vp["max_pair_deg"] >= cfg["min_pair_angular_spread_deg"]

    overall_reasons = []
    if not object_pass:
        overall_reasons.append("object_gate_fail")
    if not vp_pass:
        overall_reasons.append(
            "viewpoint_spread({:.1f}<{:.1f})".format(
                vp["max_pair_deg"], cfg["min_pair_angular_spread_deg"]))

    # viewpoint diversity fail 인 경우 어디에 시점을 추가하면 좋을지 추천
    recommendation = None
    if not vp_pass:
        try:
            recommendation = recommend_viewpoint_for_diversity(
                frames,
                target_spread_deg=cfg["min_pair_angular_spread_deg"],
                distance_m=0.30,
            )
        except Exception as exc:
            recommendation = {"available": False, "error": str(exc)}

    return {
        "frame_id": frame_id,
        "config": cfg,
        "objects": per_object,
        "viewpoint_diversity": vp,
        "viewpoint_pass": vp_pass,
        "viewpoint_recommendation": recommendation,
        "overall_pass": (object_pass and vp_pass),
...
def run_pipeline(profiles: List[ObjectProfile],
                  ...
                  dynamic_cam_ids=(),
                  calib_subdir: str = "cube_session_01/calib_out_cube",
                  pose_gate_cfg: Optional[Dict[str, float]] = None,
                  verbose: bool = True) -> Dict[str, Any]:
    """profiles 리스트를 받아 SAM masks + pose + viz 까지 일괄 수행.

    dynamic_cam_ids 에 포함된 캠은 그리퍼 부착 캠으로 간주되어, 매 프레임
    `cam{ci}/T_base_ee_<frame>.npy` + hand-eye 로 extrinsic 을 계산한다.
    """
    ...
    if verbose:
        ...
        if dynamic_cam_ids:
            dyn = ",".join(f"cam{c}" for c in dynamic_cam_ids)
            print(f" dynamic (gripper) cams: {dyn}")
        print("=" * 64)

    intrinsics, extrinsics, dynamic_calib = load_calibration(
        data_dir, intr_dir,
        dynamic_cam_ids=dynamic_cam_ids, calib_subdir=calib_subdir)
    ...
    # Tier-3 quality report
    cfg = dict(DEFAULT_POSE_GATE_CFG)
    if pose_gate_cfg:
        cfg.update({k: v for k, v in pose_gate_cfg.items() if v is not None})
    report = build_quality_report(frame_id, profiles, poses, frames, cfg)
    (pose_dir / "quality_report.json").write_text(
        json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")
    if verbose:
        vp = report["viewpoint_diversity"]
        print(f"\n[QUALITY] frame={frame_id} overall_pass={report['overall_pass']}")
        print(f"  viewpoint_spread max={vp['max_pair_deg']:.1f}deg "
              f"(min thr={cfg['min_pair_angular_spread_deg']:.1f}) "
              f"-> {'OK' if report['viewpoint_pass'] else 'FAIL'}")
        for prof in profiles:
            r = report["objects"].get(prof.name, {})
            m = r.get("metrics", {})
            tag = "OK " if r.get("pass") else "FAIL"
            if m:
                print(f"  [{tag}] {prof.name}  fit={m.get('fitness',0):.3f} "
                      f"rmse={m.get('rmse_m',0)*1000:.1f}mm "
                      f"iou={m.get('silhouette_iou',0):.3f} "
                      f"scale={m.get('scale',0):.2f}")
                if r.get("reasons"):
                    print(f"        reasons: {', '.join(r['reasons'])}")
            else:
                print(f"  [{tag}] {prof.name}  (no pose)")
        rec = report.get("viewpoint_recommendation")
        if rec and rec.get("available"):
            print(f"  recommend: gripper cam at ({rec['cam_position_m'][0]:+.2f},"
                  f"{rec['cam_position_m'][1]:+.2f},{rec['cam_position_m'][2]:+.2f})m"
                  f" looking at ({rec['look_at_m'][0]:+.2f},"
                  f"{rec['look_at_m'][1]:+.2f},{rec['look_at_m'][2]:+.2f})m"
                  f"  az={rec['azimuth_deg']:+.0f}° el={rec['elevation_deg']:+.0f}°"
                  f"  -> expected max_pair={rec['expected_max_pair_deg_after']:.1f}°")
